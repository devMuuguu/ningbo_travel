  ...
  <div class="space-y-6">
    <div>
      ...
      <p class="text-sm text-slate-400 mb-4">
        Мөр эсвэл тийз дээр дарж дэлгэрэнгүй мэдээлэл, захиалгын код болон
        газрын зургийн холбоосыг нээнэ үү.
      </p>
    ...
    <div
      class="bg-linear-to-r from-[#1b2637] to-[#26344a] border border-[#2f3f58] rounded-xl p-4 flex flex-wrap items-center justify-between gap-4"
    >
      <div class="flex items-center gap-3">
        <div
          class="w-10 h-10 rounded-lg bg-emerald-500/15 border border-emerald-500/30 flex items-center justify-center text-emerald-400"
        >
          <CheckCircle2 class="w-5 h-5" />
        </div>
        <div>
          <div class="text-xs uppercase font-mono text-slate-400">
            Баталгаажсан Нийт Тээврийн Зардал
          </div>
          <div class="text-xl font-bold font-['Oswald'] text-emerald-400">
            $1,211.88
            <span class="text-xs font-sans text-slate-400 font-normal"
              >(4 хүн)</span
            >
          </div>
        </div>
      ...
      <div
        class="text-xs font-mono text-slate-400 bg-[#131b29] px-3 py-1.5 rounded-lg border border-[#2f3f58]"
      >
    ...
    <div class="space-y-4">
      ...
      >
        ...
        >
          <div class="flex items-center gap-3 min-w-45">
            <div class="font-mono text-sm font-semibold text-amber-400 w-16">
              {{ ticket.date }}
              <span class="block text-xs font-bold text-slate-100">{{
                ticket.time
              }}</span>
            </div>
        ...
        <div
          v-if="ticket.open"
          class="border-t border-[#2f3f58] bg-[#131b29]/90 p-4 sm:p-5 space-y-4"
        >
          <!-- Ticket Stub Details Grid -->
          <div
            class="grid grid-cols-2 sm:grid-cols-3 gap-3 bg-[#f8f4ea] text-[#171f2c] p-4 rounded-xl border border-[#d6ccb6] font-sans shadow-inner"
          >
            <div
              v-for="(field, index) in ticket.fields"
              :key="index"
              class="space-y-0.5"
            >
              <div
                class="text-[11px] font-bold uppercase tracking-wider text-[#55617a]"
              >
                {{ field.k }}
              </div>
              <div
                class="text-xs sm:text-sm font-semibold text-[#171f2c] whitespace-pre-line"
                :class="{ 'text-amber-700 font-bold': field.highlight }"
              >
                {{ field.v }}
              </div>
            </div>
          ...
          <div
            v-if="ticket.copyCodes && ticket.copyCodes.length"
            class="flex flex-wrap items-center gap-2 pt-2"
          >
            <span class="text-xs text-slate-400 font-mono">Түргэн хуулах:</span>
            <CopyButton
              v-for="c in ticket.copyCodes"
              :key="c.code"
              :text="c.code"
              :label="c.label"
            />
          ...
          <div
            v-if="ticket.maps && ticket.maps.length"
            class="flex flex-wrap items-center gap-2 pt-2 border-t border-[#2f3f58]"
          >
...
const tickets = ref([
  {
    ...
    maps: [
      {
        title: "UBN Чингис хаан буудал",
        url: "https://www.google.com/maps/search/?api=1&query=Chinggis+Khaan+International+Airport",
      },
      {
        title: "PEK 北京首都国际机场",
        url: "https://www.google.com/maps/search/?api=1&query=Beijing+Capital+International+Airport",
      },
    ],
  },
  {
    ...
    fields: [
      ...
      {
        k: "Захиалгын дугаар",
        v: "1688900494199231 (Trip.com)",
        highlight: true,
      },
      {
        k: "Суудал",
        v: "Вагон 08, суудал 015F, 014F, 015D, 014D",
        highlight: true,
      },
    ...
    maps: [
      {
        title: "Beijing South Station",
        url: "https://www.google.com/maps/search/?api=1&query=Beijing+South+Railway+Station",
      },
      {
        title: "Shanghai Railway Station (上海站)",
        url: "https://www.google.com/maps/search/?api=1&query=31.2508246,121.4506144",
      },
    ],
  },
  {
    ...
    copyCodes: [{ label: "Jing'an Temple", code: "静安寺" }],
    maps: [
      {
        title: "Shanghai Home Stay Maps",
        url: "https://www.google.com/maps/search/?api=1&query=31.22211586149014,121.45399211177829",
      },
    ],
  },
  {
    ...
    fields: [
      ...
      {
        k: "Суудал",
        v: "Вагон 04, суудал 013F, 014F, 013D, 014D",
        highlight: true,
      },
    ...
    maps: [
      {
        ...
        url: "https://www.google.com/maps/search/?api=1&query=Shanghai+South+Railway+Station",
      },
      {
        title: "Ningbo Railway Station",
        url: "https://www.google.com/maps/search/?api=1&query=29.861388,121.536331",
      },
    ],
  },
  {
    ...
    fields: [
      ...
      {
        k: "Хаяг",
        v: "No. 196 Changjiang South Road, Beilun District, Ningbo",
      },
    ...
    maps: [
      {
        title: "Huihe Ruyu Hotel Maps",
        url: "https://www.google.com/maps/place/29%C2%B053'18.0%22N+121%C2%B050'23.0%22E",
      },
    ],
  },
  {
    ...
    maps: [
      {
        title: "Ningbo Lishe Airport (NGB)",
        url: "https://www.google.com/maps/search/?api=1&query=Ningbo+Lishe+International+Airport",
      },
      {
        title: "Beijing Capital Airport (PEK)",
        url: "https://www.google.com/maps/search/?api=1&query=Beijing+Capital+International+Airport",
      },
    ],
  },
...
const filteredTickets = computed(() => {
  ...
  return tickets.value.filter(
    (t) =>
      ...
      t.date.includes(q) ||
      t.fields.some(
        (f) => f.v.toLowerCase().includes(q) || f.k.toLowerCase().includes(q),
      ),
  );
});
</script>
