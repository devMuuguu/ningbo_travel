  ...
  <div class="space-y-6">
    <div>
      ...
      <p class="text-sm text-slate-400 mb-6">
        Өдөр бүрийн хөдлөх, буух, такси болон тээврийн цагийн нарийвчилсан
        зааварчилгаа.
      </p>
    </div>
    ...
    <div class="space-y-4">
      ...
      >
        <!-- Day Badge -->
        <div
          class="flex flex-wrap items-center justify-between gap-2 border-b border-[#2f3f58] pb-3"
        >
          <div class="flex items-center gap-3">
            ...
            <span
              v-if="day.highlight"
              class="px-2 py-0.5 rounded text-[11px] font-bold uppercase bg-amber-500/20 text-amber-300 border border-amber-500/30"
            >
              VNL Тэмцээний Өдөр
            </span>
          </div>
          <span
            class="text-xs font-mono text-slate-400 bg-[#131b29] px-2.5 py-1 rounded border border-[#2f3f58]"
          >
            {{ day.location }}
          </span>
        </div>
        ...
        <div
          v-if="day.steps && day.steps.length"
          class="space-y-2 pt-2 border-t border-[#2f3f58]/60"
        >
          <div
            v-for="(step, idx) in day.steps"
            :key="idx"
            class="flex items-start gap-2.5 text-xs text-slate-300"
          >
            <Clock class="w-3.5 h-3.5 text-amber-400 mt-0.5 shrink-0" />
            <div>
              <strong class="text-amber-300 font-mono mr-1">{{
                step.time
              }}</strong>
              <span>{{ step.text }}</span>
            </div>
          ...
          <span
            ...
            :class="
              chip.hot
                ...
                : 'bg-[#131b29] text-slate-300 border border-[#2f3f58]'
            "
          >
            {{ chip.text }}
          </span>
...
const days = ref([
  {
    ...
    steps: [
      ...
      {
        time: "18:35",
        text: "Улаанбаатараас Beijing чиглэлийн нислэг хөдөлнө",
      },
  ...
  },
  {
    ...
    steps: [
      {
        time: "05:20",
        text: "Такси/Didi-гээр Beijing South (北京南站) руу гарна",
      },
      {
        time: "06:15",
        text: "G3 галт тэрэгний Хаалга 10A/10B дээр бэлэн байна",
      },
